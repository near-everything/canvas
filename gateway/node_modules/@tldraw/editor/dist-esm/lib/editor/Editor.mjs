...
class Editor extends EventEmitter {
  ...
  getShapeOutlineSegments(shape) {
    return this._getShapeOutlineSegmentsCache().get(typeof shape === "string" ? shape : shape.id) ?? EMPTY_ARRAY;
  }
  ...
  _getShapePageTransformCache() {
    return this.store.createComputedCache("pageTransformCache", (shape) => {
      ...
      const parentTransform = this._getShapePageTransformCache().get(shape.parentId) ?? Matrix2d.Identity();
  ...
  getShapeMaskedPageBounds(shape) {
    ...
    if (pageMask) {
      ...
      if (corners.every((p, i) => Vec2d.Equals(p, pageMask[i])))
      ...
      switch (type) {
        ...
        case "wheel": {
          ...
          if (this.getIsMenuOpen()) {
            ...
            this.pan(info.delta);
            if (!inputs.isDragging && inputs.isPointing && originPagePoint.dist(currentPagePoint) > (this.getInstanceState().isCoarsePointer ? COARSE_DRAG_DISTANCE : DRAG_DISTANCE) / this.getZoomLevel()) {
...
__decorateClass([
  computed
], Editor.prototype, "_getShapeOutlineSegmentsCache", 1);
__decorateClass([
  computed
