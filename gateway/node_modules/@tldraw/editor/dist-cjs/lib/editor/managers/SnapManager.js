...
class SnapManager {
  ...
  _snapLines = (0, import_state.atom)("snapLines", void 0);
  get lines() {
    return this._snapLines.get() ?? import_state.EMPTY_ARRAY;
  }
  clear() {
  ...
  getSnapPointsCache() {
    ...
    return editor.store.createComputedCache("snapPoints", (shape) => {
      ...
      return snapPoints.map((point, i) => {
        const { x, y } = import_Matrix2d.Matrix2d.applyToPoint(pageTransfrorm, point);
      ...
      for (let j = i + 1; j < sortedShapesOnCurrentPageHorizontal.length; j++) {
        ...
        if (
          ...
          horizontal.push({
            ...
            endNode,
            startEdge: [
              new import_Vec2d.Vec2d(startNode.pageBounds.maxX, startNode.pageBounds.minY),
              new import_Vec2d.Vec2d(startNode.pageBounds.maxX, startNode.pageBounds.maxY)
            ],
            endEdge: [
              new import_Vec2d.Vec2d(endNode.pageBounds.minX, endNode.pageBounds.minY),
              new import_Vec2d.Vec2d(endNode.pageBounds.minX, endNode.pageBounds.maxY)
            ],
      ...
      for (let j = i + 1; j < sortedShapesOnCurrentPageVertical.length; j++) {
        ...
        if (
          ...
          vertical.push({
            ...
            endNode,
            startEdge: [
              new import_Vec2d.Vec2d(startNode.pageBounds.minX, startNode.pageBounds.maxY),
              new import_Vec2d.Vec2d(startNode.pageBounds.maxX, startNode.pageBounds.maxY)
            ],
            endEdge: [
              new import_Vec2d.Vec2d(endNode.pageBounds.minX, endNode.pageBounds.minY),
              new import_Vec2d.Vec2d(endNode.pageBounds.maxX, endNode.pageBounds.minY)
            ],
  ...
  snapTranslate({
    ...
    const nearestSnapsX = [];
    const nearestSnapsY = [];
    const minOffset = new import_Vec2d.Vec2d(snapThreshold, snapThreshold);
    ...
    const nudge = new import_Vec2d.Vec2d(
  ...
  getOutlinesInPageSpace() {
    return this.getSnappableShapes().map(({ id, isClosed }) => {
      const outline = (0, import_utils.deepCopy)(this.editor.getShapeGeometry(id).vertices);
      if (isClosed)
        outline.push(outline[0]);
      const pageTransform = this.editor.getShapePageTransform(id);
      if (!pageTransform)
        throw Error("No page transform");
      return import_Matrix2d.Matrix2d.applyToPoints(pageTransform, outline);
    });
  }
  /**
  ...
  getSnappingHandleDelta({
    ...
    for (const outline of allSegments) {
      for (let i = 0; i < outline.length - 1; i++) {
        C = outline[i];
        D = outline[i + 1];
        nearest = import_Vec2d.Vec2d.NearestPointOnLineSegment(C, D, handlePoint);
        distance = import_Vec2d.Vec2d.Dist(handlePoint, nearest);
        if (isNaN(distance))
          continue;
    ...
    if (nearestPoint) {
      ...
      return import_Vec2d.Vec2d.Sub(nearestPoint, handlePoint);
    }
    return null;
  }
  snapResize({
    ...
    if (scaleX < 0) {
      handle = (0, import_Box2d.flipSelectionHandleX)(handle);
    }
    if (scaleY < 0) {
      handle = (0, import_Box2d.flipSelectionHandleY)(handle);
    }
    ...
    if (isAspectRatioLocked && (0, import_Box2d.isSelectionCorner)(handle) && nudge.len() !== 0) {
    ...
    const snappedDelta = import_Vec2d.Vec2d.Add(dragDelta, nudge);
    const { box: snappedResizedPageBounds } = import_Box2d.Box2d.Resize(
      initialSelectionPageBounds,
      originalHandle,
  ...
  collectPointSnaps({
    ...
    for (const thisSnapPoint of selectionSnapPoints) {
      for (const otherSnapPoint of otherNodeSnapPoints) {
        const offset = import_Vec2d.Vec2d.Sub(thisSnapPoint, otherSnapPoint);
  ...
  getPointSnapLines({
    ...
    return Object.values(snapGroupsX).concat(Object.values(snapGroupsY)).map((snapGroup) => ({
      ...
      points: (0, import_utils.dedupe)(
        snapGroup.map((snap) => import_Vec2d.Vec2d.From(snap.otherPoint)).concat(snapGroup.map((snap) => import_Vec2d.Vec2d.From(snap.thisPoint))),
        (a, b) => a.equals(b)
      )
    }));
  }
...
], SnapManager.prototype, "getSnapPointsCache", 1);
__decorateClass([
  import_state.computed
...
__decorateClass([
  import_state.computed
], SnapManager.prototype, "getSnappablePoints", 1);
__decorateClass([
  import_state.computed
], SnapManager.prototype, "getVisibleGaps", 1);
__decorateClass([
  import_state.computed
], SnapManager.prototype, "getOutlinesInPageSpace", 1);
