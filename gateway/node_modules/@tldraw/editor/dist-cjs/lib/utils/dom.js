...
function preventDefault(event) {
  ...
  if (import_debug_flags.debugFlags.preventDefaultLogging.get()) {
    console.warn("preventDefault called on event:", event);
  }
}
function setPointerCapture(element, event) {
  element.setPointerCapture(event.pointerId);
  if (import_debug_flags.debugFlags.pointerCaptureTracking.get()) {
    const trackingObj = import_debug_flags.debugFlags.pointerCaptureTrackingObject.get();
    trackingObj.set(element, (trackingObj.get(element) ?? 0) + 1);
  }
  if (import_debug_flags.debugFlags.pointerCaptureLogging.get()) {
